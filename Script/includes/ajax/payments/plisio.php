...
try {
  ...
  switch ($_POST['handle']) {
    case 'packages':
      ...
      // get plisio link
      $plisio = plisio("packages", $package['price'], $package['package_id']);

      // update user
      $db->query(sprintf("UPDATE users SET plisio_hash = %s, plisio_txn_id = %s WHERE user_id = %s", secure($plisio['plisio_hash']), secure($plisio['txn_id']), secure($user->_data['user_id'], 'int')));

      // return link
      $link = $plisio['invoice_url'];
      break;
    ...
    case 'wallet':
      ...
      // get plisio link
      $plisio = plisio("wallet", $_POST['price']);
    ...
    case 'donate':
      ...
      // get plisio link
      $plisio = plisio("donate", $_POST['price'], $_POST['post_id']);
    ...
    case 'subscribe':
      ...
      // get plisio link
      $plisio = plisio("subscribe", $monetization_plan['price'], $_POST['plan_id']);
    ...
    case 'paid_post':
      ...
      // get plisio link
      $plisio = plisio("paid_post", $post['post_price'], $_POST['post_id']);
    ...
    case 'movies':
      ...
      // get plisio link
      $plisio = plisio("movies", $movie['price'], $_POST['movie_id']);
    ...
    case 'marketplace':
      ...
      // get plisio link
      $plisio = plisio("marketplace", $orders_collection['total'], $_POST['orders_collection_id']);
