...
async function init_Agora(type, token, room_name, call_id) {
  ...
  await client.join(agora_app_id, room_name, token, null);
        ...
        switch (response.call) {
          ...
          default:
            ...
            if (audio_video_provider == "twilio") {
              ...
              /* init_LiveKit */
              init_LiveKit(type, response.call['from_user_token'], response.call['room'], response.call['call_id']);
            } else if (audio_video_provider == "agora") {
              /* init_Agora */
              init_Agora(type, response.call['from_user_token'], response.call['room'], response.call['call_id']);
            }
        }
      }
    }, 'json')
    ...
    $.post(api['chat/call'], { 'do': 'answer_call', type: type, 'id': id }, function (response) {
      if (response.callback) {
        ...
        if (audio_video_provider == "twilio") {
          ...
          /* init_LiveKit */
          init_LiveKit(type, response.call['to_user_token'], response.call['room'], response.call['call_id']);
        } else if (audio_video_provider == "agora") {
          /* init_Agora */
          init_Agora(type, response.call['to_user_token'], response.call['room'], response.call['call_id']);
        }
      }
    }, 'json')
      .fail(function () {
