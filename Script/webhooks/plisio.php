...
try {
  if ($_GET['status'] == "success") {
    switch ($_GET['handle']) {
      case 'packages':
        // valid inputs
        if (!isset($_GET['plisio_hash'])) {
          _error(404);
        }
        if (!isset($_GET['package_id']) || !is_numeric($_GET['package_id'])) {
          _error(404);
        }
        if ($_GET['plisio_hash'] != $user->_data['plisio_hash']) {
          _error(404);
        }
        ...
        // check payment
        $payment = plisio_check($user->_data['plisio_txn_id']);
        if ($payment) {
          /* update user package */
          $user->update_user_package($package);
          /* log payment */
          $user->log_payment($user->_data['user_id'], $package['price'], 'plisio', 'packages');
          /* redirect */
          redirect("/upgraded");
        }
        break;
      ...
      case 'wallet':
        ...
        if ($payment && isset($_SESSION['wallet_replenish_amount'])) {
          ...
          /* log payment */
          $user->log_payment($user->_data['user_id'], $_SESSION['wallet_replenish_amount'], 'plisio', 'wallet');
          /* redirect */
      ...
      case 'donate':
        ...
        if ($payment) {
          ...
          /* log payment */
          $user->log_payment($user->_data['user_id'], $_SESSION['donation_amount'], 'plisio', 'donate');
          /* redirect */
      ...
      case 'subscribe':
        ...
        if (!$payment) {
          ...
          /* log payment */
          $user->log_payment($user->_data['user_id'], $monetization_plan['price'], 'plisio', 'subscribe');
          /* redirect */
          redirect($node_link);
        }
        break;
      ...
      case 'paid_post':
        ...
        if (!$payment) {
          ...
          /* log payment */
          $user->log_payment($user->_data['user_id'], $post['paid_price'], 'plisio', 'paid_post');
          /* redirect */
          redirect($post_link);
        }
        break;
      ...
      case 'movies':
        ...
        if (!$payment) {
          ...
          /* log payment */
          $user->log_payment($user->_data['user_id'], $movie['price'], 'plisio', 'movies');
          /* redirect */
          redirect($movie_link);
        }
        break;
      ...
      case 'marketplace':
        ...
        if (!$payment) {
          ...
          /* log payment */
          $user->log_payment($user->_data['user_id'], $orders_collection['total'], 'plisio', 'marketplace');
          /* redirect */
  ...
  // update user
  $db->query(sprintf("UPDATE users SET plisio_hash = '', plisio_txn_id = '' WHERE user_id = %s", secure($user->_data['user_id'], 'int')));
